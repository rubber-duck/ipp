...
uniform vec4 u_placement; // position.xy, size.xy in Surface metres
uniform vec4 u_clip; // min.xy, max.xy in Surface metres
uniform vec4 u_color; // straight linear fill RGBA
uniform vec4 u_border_color; // straight linear border RGBA
uniform vec4 u_box; // corner_rx, corner_ry, border_width, reserved
...
void main() {
    ...
    vec2 half_size = u_placement.zw * 0.5;
    vec2 center = u_placement.xy + half_size;
    // Clamp each explicit radius to the corresponding placed half size.
    vec2 corner = min(max(u_box.xy, vec2(0.0)), half_size);
    ...
    if (u_box.z > 0.0) {
        vec2 inner_half = max(half_size - u_box.z, vec2(0.0));
        vec2 inner_corner = max(corner - u_box.z, vec2(0.0));
        float inner = sd_round_box(offset, inner_half, inner_corner);
        border_mix = smoothstep(-edge, edge, inner);
    }
    vec3 rgb = mix(u_color.rgb, u_border_color.rgb, border_mix);
    float alpha = mix(u_color.a, u_border_color.a, border_mix) * fill * clip_coverage;
