...
Direct presentation through shapes, glyphs, drawings and bitmaps is the default. Surface authoring may opt into whole-Surface texture caching as a presentation optimization. The [distance-based policy](../../crates/ipp-core/src/world/systems/surface/cache_policy.rs) keeps nearby Surfaces direct and lowers cache resolution and maximum content refresh frequency as camera distance increases. GUI focus or active pointer interaction takes priority over distance and uses current direct presentation. Stable transitions prevent small camera movements from repeatedly changing rendering mode or cache size. Numeric thresholds and quality settings belong with implementation and its configuration.

RenderService owns the [derived GPU images](../../crates/ipp-render-gl/src/services/render/surface_cache.rs) and their refresh scheduling from evaluated Surface inputs and Host-provided time. Cached content is composited at the current Surface placement every rendered frame; reducing content refresh frequency never reduces camera or World update frequency. Unchanged content reuses its image, while pending paint changes are coalesced into the latest evaluated state at the next eligible refresh. Camera and entity placement changes alone do not repaint content unless they select a different cache resolution. GUI state, animation, layout, hit testing and semantic observations continue through their existing owners; cached pixels never become authoritative state or a client-authored asset.
