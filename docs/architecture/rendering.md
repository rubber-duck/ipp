...
Context loss suspends GPU work and restores programs/context state before demanded resources under the [immutable recovery contract](assets.md#retention-and-recovery). Resource/draw, context, World-evaluation and fatal Host failures have distinct scopes. Recoverable failures skip affected work or use fallback without destroying the Host; context loss reaches recovery even from material preparation. Validate allocations and resource uploads when they complete, and check GL errors at the end of passes whose results are kept. Frame ends check on a sampled subset of frames and after retained-storage replacement, so other errors may be attributed to a later frame, while context loss is still detected within the frame it occurs. Explicit exhaustive checks attribute each error to its failing call, with the [hot-path logging policy](runtime.md#diagnostic-logging) unchanged.
...
Dirty content updates retained CPU geometry and replaces complete bounded GPU batches through storage replacement or available storage. Submitted storage remains valid until its GPU consumers finish; reuse cannot assume a fixed number of elapsed frames. Unchanged batches remain reusable, and camera or entity placement within a stable presentation-quality selection does not regenerate local geometry. RenderService owns allocation, retirement and context recovery independently of authored World state. The Host's RenderService owns the retained GUI and glyph batches and the single [glyph atlas](../../crates/ipp-render-gl/src/services/render/glyph_atlas.rs); World identity only keys those batches and each World's published glyph demand. When the Host [forgets a World](../../crates/ipp-render-gl/src/services/render/service/lifecycle.rs), the Service releases that World's batches and demand, and atlas page reclamation considers the combined demand of all Worlds. Detailed buffer layouts, allocation policies and shader algorithms belong beside the implementation.
...
Direct presentation through shapes, glyphs, drawings and bitmaps is the default. Surface authoring may opt into whole-Surface texture caching as a presentation optimization. The [distance-based policy](../../crates/ipp-core/src/world/systems/surface/cache_policy.rs) keeps nearby Surfaces direct and lowers cache resolution and maximum content refresh frequency as camera distance increases. GUI focus or pointer interaction (hover, press or capture) takes priority over distance and uses current direct presentation. Stable transitions prevent small camera movements from repeatedly changing rendering mode or cache size. Numeric thresholds and quality settings belong with implementation and its configuration.

RenderService owns the [derived GPU images](../../crates/ipp-render-gl/src/services/render/surface_cache.rs) and their [refresh scheduling](../../crates/ipp-render-gl/src/services/render/service/surface_cache.rs) from [evaluated Surface inputs](../../crates/ipp-core/src/world/systems/render/surface_cache_inputs.rs) and Host-provided time. Cached content is composited at the current Surface placement every rendered frame; reducing content refresh frequency never reduces camera or World update frequency. Unchanged content reuses its image, while pending paint changes are coalesced into the latest evaluated state at the next eligible refresh. Content whose paint changes on every frame at the refresh cap would repaint every frame at more cost than drawing it, so RenderService [presents such a Surface directly](../../crates/ipp-render-gl/src/services/render/surface_cache.rs) after a short streak of those repaints and returns to its image once the paint settles. Camera and entity placement changes alone do not repaint content unless they select a different cache resolution. GUI state, animation, layout, hit testing and semantic observations continue through their existing owners; cached pixels never become authoritative state or a client-authored asset.
