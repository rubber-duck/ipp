...
Gallery builds also require the pinned Blender environment (`python tools/ipp.py setup blender`) and Chromium (`python tools/ipp.py setup browser --with-deps`). The build exports the maintained packed scenes into `target/gallery-platformer-assets/` and `target/gallery-gui-assets/projector/`; Platformer World serialization uses the matching worker/WASM contract. No KayKit archive download or projector texture rebake is needed. Runtime exports and authoring previews stay under ignored `target/`, while editable Blender/SVG sources and licenses remain in Git. Pages CI installs these prerequisites before building.
