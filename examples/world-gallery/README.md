...
The GUI Demo page converts its vector mark during the gallery build and mounts a real `@ipp/react/gui` application with `IppCanvas.guiInput`. Core layout and input route controls against the current camera. Authoritative routing keeps gestures that start on the panel in GUI, while pointer and wheel input that misses every panel drives the ordinary gallery camera controls; each gesture keeps its initial owner across panel boundaries.
