...
Ordinary gallery builds export the packed scene without rebaking. Meshes, textures and `projector.json` are generated under ignored `target/gallery-gui-assets/projector/` alongside the converted SVG drawings. Generate that product directly with `python tools/ipp.py build gallery-gui-assets`.
...
The authored frustum is an open rounded-rectangle loft with 56 triangles. It already accounts for the scene's 2.2 body scale: its near cross-section is at Z = 1.0714 with half-width/height 0.242, and its far cross-section is at Z = 5.1 with half-width 2.569 and half-height 1.659. The far edge follows the GUI frame's centerline, including its quadratic Bézier corners with 0.119-metre horizontal and vertical spans, sampled with six segments per corner. These are the SVG's quadratic corners, not circular arcs. The near corner spans scale by the corresponding half-size ratios, preserving the same normalized contour at both ends for straight shader rays. The near profile fits within the scaled optical disk. Apply the common projector rotation/translation to the frustum at scale 1; apply that rotation/translation to the body at scale 2.2. The target Surface is 7.4 × 4.8 at scale 0.70, centered at the frustum's far plane with the same rotation; the frame centerline is inset 0.021 metres from its outer bounds. UV U follows distance around the far perimeter, with corresponding values at the near perimeter. UV V is 0 at the port and 1 at the Surface after exporter V conversion. No cap obscures the lens or panel. Changing these scene dimensions or the GUI frame profile requires regenerating this authored geometry.
