...
import { lifecycleHostCases } from "../integration/lifecycle-subscriptions-cases.js";
import {
...
test("browser lifecycle subscriptions end at detach and client close", {
  timeout: 30_000,
}, async (context) => {
  const workspace = resolve(process.cwd());
  const profile = resolve(workspace, "target/world-host-build/wasm");
      ...
      environment.execute("lifecycle detach and close", {}, () =>
        environment.page.evaluate(async (urls) => {
          ...
          const { lifecycleSessionIsolation } = await import(
            `${urls.origin}/dist/tests/integration/scenarios/lifecycle-subscriptions.js`
          );
          ...
          try {
            return await lifecycleSessionIsolation(host);
          } finally {
            await host.close();
          }
        }, environment.urls),
