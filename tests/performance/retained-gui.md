...
Glyph population is bounded per frame by time rather than a fixed count. Each frame populates at least 32 missing entries, then as many as a 2 ms budget covers at the estimated per-glyph cost, up to 512 (`GlyphPopulationBudget` in [the glyph atlas](../../crates/ipp-render-gl/src/services/render/glyph_atlas.rs)). Native renderers refine the estimate from measured population passes; WebGL keeps a fixed estimate because its draws execute in the GPU process. Cold text within the budget therefore samples the atlas from its first frame instead of showing analytic glyphs over several frames. The cold counters accumulate over every rendered tick, so they do not depend on how population splits across frames.
...
Analytic builds lack the retained counters and report them unavailable, never as zero. The GUI counter `guiResidentBytes` is the per-Surface GPU storage holding retained box and glyph batches, including the room each slot reserves to grow; `glyphResidentBytes` counts atlas pages separately, at one byte per texel of single-channel coverage. Boxes and glyph quads share the `GuiVertex` layout defined in [the retained batch implementation](../../crates/ipp-render-gl/src/services/render/gui_batch.rs).
...
Each streamed-update sample records two latencies from the same update. `elapsedMs` is Node-side wall time from requesting the React update to having the next completed frame, so it also contains the harness round trips to the page, base64 pixel transfer and PNG writing. `inPage.updateToReadbackMs` is measured inside the page from starting the update until the next completed frame's pixels arrive there; it splits into `updateMs` (until the runtime acknowledges the update) and `captureMs` (requesting the next completed frame until its pixels arrive), and `readbackMs` is the worker's wait to finish GPU work and read the pixels back. The in-page interval therefore spans the frame that applies the update and the frame that presents it. `comparison.json` summarises each measure's median and 90th percentile per build under `timings`, with the definitions beside them in `timingDefinitions`; per-sample values stay in each build's `samples`.

These timings still include client, transport, scheduling and readback and must not be reported as isolated rendering cost or FPS. Software GL proves correctness in its environment, not mobile performance.
